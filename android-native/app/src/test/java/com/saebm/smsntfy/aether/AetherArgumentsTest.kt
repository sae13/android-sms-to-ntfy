...
class AetherArgumentsTest {
    private val h3 = AetherRoutes.supported.first { it.id == "masque-h3" }
    private val h2 = AetherRoutes.supported.first { it.id == "masque-h2" }
    private val wireGuard = AetherRoutes.supported.first { it.id == "wireguard" }
    private val gool = AetherRoutes.supported.first { it.id == "gool" }
    ...
    fun eachRouteUsesOnlyArgumentsSupportedByThePinnedCli() {
        val binary = "/data/user/0/com.saebm.smsntfy/lib/libaether.so"
        val endpoints = AetherEndpointPolicy.endpoints(false)
        val stateDirectory = "/data/user/0/com.saebm.smsntfy/no_backup/aether"
        ...
        assertTrue(commands[2].containsAll(listOf("--scan", "turbo")))
        assertTrue(commands[3].containsAll(listOf("--scan", "balanced")))
    ...
    fun publicModePassesBothWildcardListenerAddressesToThePinnedCli() {
        val command = AetherArguments.build(
            "/data/user/0/com.saebm.smsntfy/lib/libaether.so",
            h3,
            AetherEndpointPolicy.endpoints(publicProxy = true),
            "/data/user/0/com.saebm.smsntfy/no_backup/aether"
        )
